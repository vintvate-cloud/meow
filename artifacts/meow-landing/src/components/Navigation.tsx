...
export function NavItem({ icon, label, active = false, isMobile = false, isNavbar = false, href }: { icon: any, label: string, active?: boolean, isMobile?: boolean, isNavbar?: boolean, href?: string }) {
  const content = (
    <>
      {isMobile ? (
        <div className={`flex flex-col items-center gap-1 transition-all ${active ? 'text-[#101828] dark:text-foreground' : 'text-gray-300'}`}>
          <div className={active ? 'scale-110' : ''}>{icon}</div>
          <span className="text-[10px] font-black uppercase tracking-tighter">{label}</span>
        </div>
      ) : isNavbar ? (
        <div className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all group ${active ? 'bg-[#D9FF3F] text-[#101828] dark:text-foreground' : 'hover:bg-gray-50 dark:bg-muted text-gray-400 hover:text-[#101828] dark:text-foreground'}`}>
          <div className={active ? '' : 'group-hover:scale-110 transition-transform'}>{icon}</div>
          <span className="font-bold text-sm">{label}</span>
        </div>
      ) : (
        <div className={`flex items-center gap-3 w-full p-3 rounded-2xl transition-all group ${active ? 'bg-[#D9FF3F] text-[#101828] dark:text-foreground' : 'hover:bg-gray-50 dark:bg-muted text-gray-400 hover:text-[#101828] dark:text-foreground'}`}>
          <div className={active ? '' : 'group-hover:scale-110 transition-transform'}>{icon}</div>
...
export function TopNavbar() {
  ...
  return (
    <nav className="hidden md:flex w-full bg-white dark:bg-card dark:text-card-foreground border-b border-gray-100 dark:border-border items-center justify-between px-8 py-4 z-50 sticky top-0 shadow-sm">
...
export function BottomNavbar() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-card dark:text-card-foreground border-t border-gray-100 dark:border-border px-6 py-3 flex justify-between items-center z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
      ...
      <Link href="/create-event">
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center -translate-y-6 shadow-2xl border-4 border-[#F3F0E8] dark:border-background transition-transform active:scale-90" style={{ backgroundColor: '#101828' }}>
